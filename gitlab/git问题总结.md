...
tmpfs           7.5G   28K  7.5G   1% /dev/shm
tmpfs           7.5G  6.5M  7.5G   1% /run
tmpfs           7.5G     0  7.5G   0% /sys/fs/cgroup
/dev/vda1        99G   79G   16G  84% /
...
1. 查找是哪个文件夹占用过大
   ...
   du: cannot access ‘./proc/10103/task/10103/fd/4’: No such file or directory
   du: cannot access ‘./proc/10103/task/10103/fdinfo/4’: No such file or directory
   du: cannot access ‘./proc/10103/fd/3’: No such file or directory
   du: cannot access ‘./proc/10103/fdinfo/3’: No such file or directory
   2.4G    ./opt
   2.4G    ./usr
   74G     ./var 
   79G     .
   
   2. 可以看到最大文件夹为var 进入再次查找最大
   # cd ./var
   # du -h --max-depth=1 |grep G|sort -n
   1.1G    ./log
   72G     ./opt
   ...
   3. 由于opt下只有一个gitlab 进入到gitlab 进入再次查找最大
   # cd /var/opt/gitlab
   # du -h --max-depth=1 |grep G|sort -n
   1.1G    ./postgresql
   3.4G    ./git-data
   ...
   4. 最终发下是prometheus的data下的wal文件夹过大
   ```

2. 根据以上找到/var/opt/gitlab/prometheus占用最大 是gitlib的监控

## 解决方案

1. 扩大磁盘
2. 修改prometheus的data的磁盘空间，改为更大： 'storage.tsdb.path' => "/var/opt/gitlab/prometheus/data"
3. 关掉prometheus：`prometheus_monitoring['enable'] = false`
4. 修改prometheus的data保存时间，让系统自动执行自动清理
...
1. 停止gitlab 备份配置文件
   ...
   ```
   # gitlab-ctl stop
   # cp /etc/gitlab/gitlab.rb /etc/gitlab/gitlab.rb.20220504
   ```

2. 修改prometheus的data保存时间
   ...
   external_url 'http://gitlab.yzltech.com:34678'
   ...
   gitlab_rails['gitlab_email_from'] = '414370365@qq.com'
   gitlab_rails['gitlab_email_display_name'] = 'yzlgitlab'
   ...
   gitlab_rails['smtp_port'] = 465
   gitlab_rails['smtp_user_name'] = "414370365@qq.com"
   gitlab_rails['smtp_password'] = "nembftdzamhebhai"
   ...
   -bash: clean: command not found
   [root@git wal]# clane
   -bash: clane: command not found
   [root@git wal]# clena
   -bash: clena: command not found
   [root@git wal]# cl
   clear           clock           clockdiff       cloud-id        cloud-init      cloud-init-per  
   [root@git wal]# clear 
   [root@git wal]# cat /etc/gitlab/gitlab.rb
   ## GitLab configuration settings
   ##! This file is generated during initial installation and **is not** modified
   ##! during upgrades.
   ##! Check out the latest version of this file to know about the different
...
4. 最后查看空间是否释放
   ...
   devtmpfs        7.5G     0  7.5G   0% /dev
   tmpfs           7.5G   28K  7.5G   1% /dev/shm
   tmpfs           7.5G  1.2M  7.5G   1% /run
   tmpfs           7.5G     0  7.5G   0% /sys/fs/cgroup
   /dev/vda1        99G   12G   83G  13% /
   tmpfs           1.5G     0  1.5G   0% /run/user/0
   ```
